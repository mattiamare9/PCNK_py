"""
SPM experiment — Python port of SPM.jl (Torch-only version)
...
- Supports torch-based kernel factories:
    uniform_kernel, plane_wave_pin_kernel, directed_residual_pin_kernel
- Includes training via LBFGS for neural kernels (mirrors Julia Optimization.solve(LBFGS)).
...
def train_kernel_lbfgs(
    ...
    """
    Train the neural weights of a kernel (if any) using LBFGS.
    Equivalent to Julia's Optimization.solve(..., LBFGS()) call.

    Parameters
    ----------
    kernel : nn.Module
        Kernel module with trainable parameters (e.g. NeuralWeightPlaneWaveKernel).
        The PyTorch module containing trainable weights (e.g., σ, β, k).
    X : torch.Tensor
        Training coordinates, shape (M, 3)
    y : torch.Tensor
        Target complex values, shape (M,)
    lam : float
        Regularization term "lambda" for ridge regression inside loss.
    max_iter : int
        Max iterations for LBFGS.
    """
    if not any(p.requires_grad for p in kernel.parameters()):
        return  # nothing to train

    optimizer = torch.optim.LBFGS(kernel.parameters(), lr=0.8, max_iter=max_iter)
    # ℓ=∥y−Kα∥^2+λ∥α∥^2
    # α = (K + λI)^{-1}y
    def closure():
        optimizer.zero_grad()
        ...
        loss = torch.mean(torch.abs(K @ alpha - y) ** 2)
        loss.backward()
        return loss
    # In PyTorch, the optimizer.step() method for most optimizers 
    # (like SGD or Adam) executes one single update. However, for LBFGS,
    # the step() method performs the entire training sequence up to convergence or max_iter.
...
def run_spm(
    ...
    *,
    ...
    grid = np.asarray(data.xyz_pointwise)  # optional
    ...
    grid_t = _as_torch(grid, torch.float64, tdev)
    ...
    for kname, factory in kernel_factories.items():
        ...
        for f_idx, freq in enumerate(freqs):
            ...
            # --- training stage ---
            if isinstance(kernel, nn.Module) and any(p.requires_grad for p in kernel.parameters()):
                print(f"[{kname}] Training kernel at {freq:.1f} Hz...")
                train_kernel_lbfgs(kernel, X, Y[f_idx, :], lam, max_iter=10)
